import java.util.Scanner; // Importa as classes Java necessárias // Importa a classe ou pacote necessário para o funcionamento do programa

public class Comprar extends Cliente{ // Define a classe Comprar que estende a classe Cliente // Declaração da classe principal que herda de Cliente, responsável pela lógica de compra

Menu menus = new Menu(); // Atribuição de valor: a variável recebe um valor ou o resultado de uma expressão
Scanner entrada = new Scanner(System.in); // Cria um objeto Scanner para capturar a entrada do usuário

public void cadastrar() { // Método para registrar os dados do cliente (nome e idade) // Declaração de um método público que define uma função específica do programa
System.out.println("=================================="); // Exibe saída no console
System.out.println("||FAÇA SEU CADASTRO PARA COMEÇAR||"); // Exibe saída no console // Exibe uma mensagem ou instrução para o usuário no console
System.out.println("=================================="); // Exibe saída no console

System.out.println("\nQual seu nome?"); // Exibe saída no console
setNome(entrada.nextLine()); // Captura a entrada do usuário e armazena o valor na variável correspondente

if (getNome().isEmpty()) { // Estrutura condicional: verifica se a condição entre parênteses é verdadeira

while (getNome().isEmpty()) { // Estrutura de repetição: executa o código repetidamente enquanto a condição for verdadeira
...
System.out.println("Encerrando o programa..."); // Exibe saída no console
                System.exit(0);
} else { // Bloco alternativo: executado se a condição do 'if' anterior for falsa
...
System.out.println("\nVocê tem R$" + getSaldo() + "\n"); // Exibe saída no console
menus.menuBebida(); // Cria uma instância da classe Menu para exibir opções ao usuário
